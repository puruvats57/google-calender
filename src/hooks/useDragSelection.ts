...
export function useDragSelection(onSelect: (startIdx: number, endIdx: number) => void) {
  const state = useRef<{ dragging: boolean; startIdx: number | null }>({
    dragging: false,
    startIdx: null
  });
  ...
  useEffect(() => {
    const onPointerUp = () => {
      if (state.current.dragging && state.current.startIdx !== null) {
      ...
      state.current = { dragging: false, startIdx: null };
